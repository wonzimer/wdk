...
Cryptomedia is the foundational primitive of the Wonzimer Protocol.
...
The `mint` function on a `Wonzimer` instance expects two parameters: `MediaData` and `BidShares`
...
const tx = await wonzimer.mint(mediaData, bidShares)
